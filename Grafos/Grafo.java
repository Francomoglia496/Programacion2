...
public class Grafo {
    ...
    /*public int caminoMasCorto(Nodo origen, Nodo destino){

        Nodo aux = origen;
        int costo = obtenerCosto(aux, aux.getVecinos().get(0));
        int costoTotal = 0;

        while(aux != destino){

            for (Nodo r : aux.getVecinos()){

                if (costo < obtenerCosto(aux, r) ){
                    costoTotal += costo;
                }else if (costo > obtenerCosto(aux, r)){
                    costoTotal += obtenerCosto(aux, r);
                }

                costo = obtenerCosto(aux, r);
            }
