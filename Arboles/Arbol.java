...
public class Arbol {
    ...
    * */
    ...
    public void BFSArbol(Nodo raiz) //SE RECIBE LA RAIZ DEL ARBOL
    {
	...
	int cont = 0;	
        ...
        while (!nodeQueue.isEmpty()){
            ...
            if (nodeQueue.peek().getHojaIzquierda() != null) {
                nodeQueue.add(nodeQueue.peek().getHojaIzquierda());
            }
            if (nodeQueue.peek().getHojaDerecha() != null) {
                nodeQueue.add(nodeQueue.peek().getHojaDerecha());
            }

            /*LinkedList<Nodo> colaAux = new LinkedList<Nodo>();
            colaAux.add(nodeQueue.peek().getHojaIzquierda());
            colaAux.add(nodeQueue.peek().getHojaDerecha());*/

            System.out.println(nodeQueue.poll() + " " + " nivel: " + cont);

	    cont++;

            /*for (Nodo aux : colaAux){
                if (!colaAux.contains(aux)){
                    nodeQueue.add(aux);
                }
            }*/
